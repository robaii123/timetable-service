...
public class MappingService {
    ...
    /** Request DTO -> Planning problem (domain) */
    public WeeklySchedule toProblem(TimetableRequest req) {
        ...
        // Build planning entities (sessions). Typically you receive requested sessions to place.
        List<Session> sessions = req.sessions().stream()
                .map(d -> {
                    // resolve references by id (here we assume DTO already embeds full objects;
                    // if you only have IDs, replace with lookups)
